...
## 2026-08-03 (2) — Vuelve a hornear el spawn del auto a coordenadas exactas (FindRoadTip volvió a fallar, hasta con la escena limpia)

Owner: con la escena ya sincronizada limpia (changeset 124, un solo
`PavedRoad_Surface` real), `FindRoadTip()` volvió a fallar -- el auto
apareció lejos del camino otra vez. Van 2 veces que esta técnica
(promediar vértices cerca de la punta) agarra un punto que no es el
centro real, incluso con datos limpios -- no es un problema de
duplicados, es que la técnica en sí no es confiable para este mesh.

Vuelto a horneado directo (mismo criterio que `Seat_RearMid`): el owner
voló hasta la punta real de la ruta ya sincronizada y pasó
Position (1868.026, 17.04301, 10.39595) / Rotation Y=-104.379 tal cual.
`FindRoadTip()` sacado del todo (quedó sin uso, 2 intentos fallidos son
suficiente evidencia de que no vale la pena seguir persiguiendo esta
técnica para este mesh en particular).

**Nota para el futuro:** si el compañero vuelve a mover/reemplazar el
mesh de la ruta, este spawn se va a desalinear de nuevo -- va a hacer
falta repetir este mismo paso (volar hasta la punta, pasar
Position/Rotation nueva). No vale la pena intentar automatizarlo de
nuevo con lectura de mesh; pedir la coordenada a mano es más rápido y
confiable que cualquier heurística geométrica para este caso.
