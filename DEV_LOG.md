...
## 2026-08-03 (3) — Conecta SnapToRoadExtensionTip() del compañero (estaba escrito pero nunca se llamaba)

Owner: "necesito los scripts de mi amigo" -- encontrado en Plastic
(changesets 125-127, nunca bajados) que el compañero escribió su PROPIO
sistema en `CarBuilder.cs`: `SnapToRoadExtensionTip()`. Mucho más sólido
que todo lo intentado esta sesión -- reconstruye spawn, rotación Y LOS
WAYPOINTS COMPLETOS (hasta el giro y frenada al lado de un surtidor
real) leyendo la geometría VIVA de la escena:

- Junta la línea central de TODAS las piezas "PavedRoad_Surface" +
  "PavedRoad_Surface (N)" -- estas últimas NO eran duplicados basura
  como asumí antes, son extensiones que el owner agregó A MANO
  (`RoadsideBuilder`/`ApplySavedLayout` ya las deja en su lugar real).
- Ordena por X (la punta lejos primero, la YPF al final).
- Busca la YPF y los surtidores REALES en la escena (ya movidos/
  escalados a mano), no coordenadas de MapLayout.cs.
- Arma waypoints: derecho por la ruta → gira en la entrada real →
  frena al lado de un surtidor de verdad.

**El problema no era el método en sí -- estaba escrito pero NUNCA se
llamaba desde ningún lado** (se perdió la conexión en algún merge).
Cableado ahora en `MapGenerator.Generate()`, justo después de
`ApplySavedLayout()` (mismo lugar que indica el comentario del
compañero en el código).

**Pendiente, no crítico:** el código propio de esta sesión en
`CarBuilder.Build()` (TraceRoadByRaycast/SmoothPath/IsAsphalt, el
spawn horneado, los waypoints armados ahí) quedó redundante --
`SnapToRoadExtensionTip()` corre DESPUÉS y pisa todo eso. No rompe nada
(solo cómputo de más), pero se puede limpiar en otra pasada.
...
## 2026-08-03 — Auto: opening-drive reconstruido desde la geometría REAL de la escena

El owner arma la ruta A MANO y no solo extendió el asfalto (`PavedRoad_Surface (1)`),
sino que corrió TODO el corredor: el asfalto original quedó en **Z=-143** y la YPF en
**(449, -71.3, escala 2)**. Los waypoints del opening-drive los calculaba `CarBuilder.Build`
con coordenadas de CÓDIGO (`MapLayout.PavedRoute` en Z≈40, `YpfStation` sin mover), así que
al pasar de la extensión (bien puesta) al tramo original (movido) el auto se iba de la ruta
("en el cruce se tira para la derecha").

**Fix:** `CarBuilder.SnapToRoadExtensionTip(mapRoot)` (llamado en `MapGenerator.Generate`
**después** de `ApplySavedLayout`, con todo ya en su lugar real) ahora **reconstruye TODO
el recorrido desde la escena**, sin coordenadas hardcodeadas:
1. Junta la línea central de TODAS las piezas de asfalto vivas (base + extensiones) como
   `pieza.TransformPoint((x, RoadY, PavedRouteZAt(x)))` — el mesh tiene sus vértices en
   espacio-mundo de la ruta (RoadsideBuilder), así que esto da la central exacta ya movida/rotada.
2. Ordena los puntos por X mundo (punta lejana → YPF).
3. Punto de ENTRADA = centro real de la YPF (`ML_009_EstacionYPF`) + un **offset fijo**
   `YpfEntryOffset (+59.65,-18.13)` — el owner lo marcó una vez con el TEST_PLAYER en
   (508.65,-89.43) con la YPF en (449,-71.3). Arma waypoints: recto por la ruta hasta ~15m
   antes de ese X, después dobla y para ahí → "sigue derecho y recién ahí dobla y entra".
   OJO: se guarda RELATIVO a la YPF (sigue a la estación si se mueve) pero **NO** se lee del
   TEST_PLAYER vivo — ese es el spawn del jugador y el owner lo mueve por otros motivos (lo
   puso al inicio del mapa y el auto se fue hasta los árboles). Para correr dónde entra el
   auto, tocar `YpfEntryOffset`.
4. Punto FINAL: **al lado de un surtidor**. Busca dentro de la estación cualquier objeto con
   `pump`/`surtidor`/`dispenser` en el nombre (posición viva ya movida/escalada), toma el más
   cercano a la entrada y para a `ParkBesidePumpDist`=3.5m de él, del lado por el que llega el
   auto (no encima — la estación tiene colliders). Si NO detecta surtidores (o falta el
   modelo), igual avanza ~22m hacia el centro para no frenar en el borde de la tierra. El log
   dice cuántos surtidores encontró. Waypoints finales: ...ruta → entrada → al lado del surtidor.
   (CarAutoDrive frena solo en el ÚLTIMO waypoint — no hay stop por posición — así que si el
   auto frenaba en el borde era porque el waypoint del surtidor no se agregaba.)

Se adapta solo si el owner mueve la ruta o la YPF (y hace Save Map Layout). Spawn ~15m
adentro de la punta mirando a la estación. Limitación: asume ruta x-monótona (la actual lo es).

---

## 2026-08-03 — YPF: líneas de estacionamiento + fix Play From Scene View (spawn de red)

**Líneas de estacionamiento (YPF).** Nuevo `ParkingLines()` en `AreaPoiBuilder.YpfStation`,
llamado justo después de crear el playón. Hornea un grupo `LineasEstacionamiento`
(hijo de `EstacionYPF`) con cubos finitos (0.15×0.05×5m) apenas sobre el asfalto
(`padTop + 0.03`, sin colisión): 6 rayas **blancas** divisorias (= 5 lugares) + 1 tope
+ 2 **amarillas** de ejemplo. Cada raya es un objeto suelto para reacomodar a mano;
mover el grupo traslada todo. Se re-hornea igual en cada Generate; posiciones movidas
a mano se fijan con **Save Map Layout**.

**Colisiones de la estación YPF.** Nuevo `AddMeshColliders()`, llamado sobre el
`EstacionModelo` tras `HideCatalogClutter`. Agrega una `MeshCollider` no-convexa por
cada mesh **activo** del modelo (tienda/techo/columnas/surtidores/cartel) — antes el
modelo entraba sin colisión y se podía atravesar. Saltea meshes inactivos (cajones
ocultados) y los que ya tengan collider.

**Piso de cemento sólido.** El playón (`PlayonAsfalto`) ahora **conserva** su BoxCollider
(antes se le hacía `DestroyCol`) para que el jugador se pare encima y no se hunda al
terreno. Para no repetir el bug del auto trabado en la entrada, se bajó el playón casi al
ras del terreno (asoma **~0.02m** en vez de 0.12): sigue tapando el suelo y da piso
sólido, pero el escalón es tan bajo que el auto lo cruza sin frenarse.

**Play From Scene View (`Assets/editor/MapGenerator/PlayFromSceneView.cs`).** Ya no
teletransporta apenas entra a Play — el jugador lo spawnea **NetGameSpawner** en red
DESPUÉS de conectar, y encima el arranque tiene pantalla "Crear sala (HOST)". Ahora
**espera**: si hay `NetworkManager` en escena, poll hasta que exista
`LocalClient.PlayerObject` (tu personaje real, tras crear la sala), y recién ahí lo
mueve al foco de la Scene view (owner-authoritative → sincroniza solo). **No** cae al
`TEST_PLAYER` del menú (era el bug: movía al temporal). Sigue 100% editor-only y
gateado por el toggle (apagado = spawn del inicio intacto). Fallback single-player
(sin NetworkManager): TEST_PLAYER/MapExplorer.
