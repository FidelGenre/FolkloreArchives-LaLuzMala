        ...
        static List<TreeInstance> ScatterTrees(int realTreeCount, int greenIndex, int dryIndex)
        {
            ...
            for (float x = 10f; x < MapLayout.MapSizeX - 10f; x += step)
            {
                for (float z = 10f; z < MapLayout.MapSize - 10f; z += step)
                {
                    ...
                    if (Vector2.Distance(p, MapLayout.OldLadyBarnCenter) < 6f) continue; // ni sobre el galpón
        ...
        static void SetupGrass(TerrainData td)
        {
            ...
            for (int zi = 0; zi < res; zi++)
            {
                for (int xi = 0; xi < res; xi++)
                {
                    ...
                    if (Vector2.Distance(p, MapLayout.OldLadyBarnCenter) < 4.5f) continue; // bajo el galpón
