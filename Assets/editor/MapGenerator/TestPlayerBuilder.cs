    ...
    public static class TestPlayerBuilder
    {
        ...
        static void BuildDogAndParty(GameObject player, GameObject personCamGO, Vector2 playerXZ, Terrain t)
        {
            ...
            if (vcShader != null)
            {
                ...
                if (dogMat.HasProperty("_PsxSnap")) dogMat.SetFloat("_PsxSnap", 150f);         // temblor PS1 (más suave)
                if (dogMat.HasProperty("_PsxColorLevels")) dogMat.SetFloat("_PsxColorLevels", 40f); // bandas de color (más suaves)
            }
